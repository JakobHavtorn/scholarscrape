...
def compute_self_citation_count(author_id: str, ss: SemanticScholarAPI = None) -> int:
    ...
    author_df["self_cite_factor"] = author_df["self_cites"] / author.citation_count
    author_df["pseudo_self_cite_factor"] = author_df["pseudo_self_cites"] / author.citation_count
...
if __name__ == "__main__":
    ...
    for name, df in author_df.items():
        ...
        pscf = f"PSCF={df.pseudo_self_cite_factor.item():5.3f}"
        h_index = f"h={df.h_index.item():<3d}"
        h_index_corrected = f"h_corrected={df.h_index_without_self_cites.item():<3d}"
        i_index = f"i={df.i10_index.item():<3d}"
        i_index_corrected = f"i_corrected={df.i10_index_without_self_cites.item():<3d}"
